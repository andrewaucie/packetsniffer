 ...
 * Orchestrates CLI parsing, pcap setup, thread management, and UI lifecycle.
 */
...
int main(int argc, char *argv[]) {
    ...
    // Determine optimal thread count (reserve one core for UI)
    ...
    // --- Setup pcap Capture Session ---
    // Must be done before ncurses initialization to display errors properly
    if (pcap_file != NULL) {
    ...
    // --- Initialize ncurses ---
    // Must be done after pcap setup to avoid mangling error messages
    if (!init_ncurses()) {
    ...
    // Configure and draw initial windows before launching threads
    int main_h, main_w;
    ...
    // --- Launch Threads ---
    // Consumer threads process packets; producer thread captures them
    ...
    // --- Run the UI Loop ---
    // Runs in main thread; blocks until user quits or capture completes
    ui_loop();
